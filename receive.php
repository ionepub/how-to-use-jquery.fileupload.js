<?php
    /* 允许上传的文件类型 */
    $allow_file_types = '|GIF|JPG|PNG|BMP|SWF|DOC|DOCX|XLS|XLSX|PPT|PPTX|MID|WAV|ZIP|RAR|PDF|CHM|RM|TXT|';
    $file_dir = '/data/uploads/';
    ...
    for($i=0; $i < count($up_files['name']); $i++){
        if (empty($up_files['error'][$i]) || (!isset($up_files['error'][$i]) && isset($up_files['tmp_name'][$i]) && $up_files['tmp_name'][$i] != 'none'))
        {
            ...
            if(!file_exists('.'.$file_dir)){
        		...
        		//移动文件
        		$target_file = '.'.$file_dir.'/'.urldecode($up_files['name'][$i]);
        		if(!move_uploaded_file($up_files['tmp_name'][$i], $target_file)){
        		    continue; //失败
        		}else{
        		    $result[] = array(
                    "name" => urldecode($up_files['name'][$i]),
                    "url"  => $file_dir.'/'.urldecode($up_files['name'][$i])
                );
        		}
        }
    }
